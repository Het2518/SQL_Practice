  ...
  useEffect(() => {
    ...
    const fetchLeaders = async () => {
      ...
      try {
        ...
        if (mounted && data) setLeaders(data.data.leaderboard ?? []);
      } catch (err) {
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {leaders.map((leader, index) => {
                const isMe = currentUser && String(currentUser.id) === String(leader.userId);
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
                        ...
                        {leader.maxStreak > leader.currentStreak && (
                          <span style={{ color: 'var(--muted)' }}>Max: {leader.maxStreak}</span>
                        )}
                      </div>
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 16, fontWeight: 600, color: leader.currentStreak > 0 ? 'var(--text)' : 'var(--muted)' }}>
                      <Flame size={18} color={leader.currentStreak > 0 ? 'var(--error)' : 'currentColor'} />
