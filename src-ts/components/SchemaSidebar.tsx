...
export function SchemaSidebar({ dbName, onPreviewTable }: SchemaSidebarProps) {
  const [expandedTables, setExpandedTables] = useState<Set<string>>(new Set());
  const [expandedSection, setExpandedSection] = useState<'schema' | 'resources'>('schema');
  ...
  return (
    ...
    >
      ...
      >
        {(['schema', 'resources'] as const).map((s) => (
          <button
            key={s}
      ...
      <div style={{ padding: '10px 10px', flex: 1, overflowY: 'auto' }}>
        {expandedSection === 'schema' ? (
          <>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Tables
            </div>
            {dbInfo.tables.map((table) => {
              const isExpanded = expandedTables.has(table.name);
              return (
                <div key={table.name} className="schema-table-item" style={{ borderBottom: '1px solid var(--border)' }}>
                  <div
                  ...
                  >
                    <span style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>{table.name}</span>
                    <button 
                    ...
                    <span style={{ color: 'var(--primary-hover)', fontSize: 10, transition: 'transform 0.15s', transform: isExpanded ? 'rotate(90deg)' : 'none' }}>▶</span>
                  </div>
                  {isExpanded && (
                    <div style={{ padding: '0 14px 10px 14px', background: 'var(--surface-2)' }}>
                      {table.columns.map((col) => (
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, overflow: 'hidden' }}>
                            {col.isPrimaryKey ? (
                              <span title="Primary Key" style={{ fontSize: 13, lineHeight: 1 }}>🔑</span>
                            ) : col.isForeignKey ? (
                              <span title="Foreign Key" style={{ fontSize: 13, lineHeight: 1 }}>🗝️</span>
                            ) : (
                              <span style={{ width: 13 }} />
                            )}
                            <span style={{ color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {col.name}
                              {col.isNullable && <span style={{ color: 'var(--muted)', fontSize: 10, marginLeft: 4 }}>?</span>}
                            </span>
                          </div>
                          <span style={{ color: 'var(--text-secondary)', fontSize: 11, fontWeight: 600, flexShrink: 0, paddingLeft: 8 }}>
                            {col.type}
                          </span>
                        </div>
                      ))}
            ...
            <div style={{ padding: '16px 14px' }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Key Concepts
              </div>
